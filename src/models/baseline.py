...
def hottest_items(ui_matrix: pd.DataFrame, df_items: pd.DataFrame, k: int = 10):
    ...
    genre_cols = df_items.drop(
        columns=["item_id", "movie_title", "release_date", "imdb_url"]
    ).columns.to_list()

    df = ui_matrix.merge(df_items[["item_id"] + genre_cols], how="left", on="item_id")
    ...
    user_genre = (
        df.groupby("user_id")[genre_cols].sum().idxmax(axis=1).reset_index(name="genre")
    )

    # dict of items seen per user
    rated_items = df.groupby("user_id")["item_id"].apply(set).to_dict()
    ...
    for _, row in user_genre.iterrows():
        ...
        top_items = [
            item
            for item in items_by_genre[genre]
