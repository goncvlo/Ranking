...
def feature_engineering(dataframes: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    user_df["u_age"] = (user_df["u_age"] - user_df["u_age"].mean()) / user_df[
        "u_age"
    ].std()
    ...
    item_df = item_df.rename(
        columns={
            col: (
                f"i_genre_is_{col}"
                if col not in ["item_id", "i_release_year", "i_release_month"]
                else col
            )
    ...
    genre_shares.columns = [
        f"u_{col.split('_')[-1]}_rating_share" for col in genre_cols
    ]
    for col in genre_shares.columns:
        genre_shares[col] = (
            genre_shares[col] - genre_shares[col].mean()
        ) / genre_shares[col].std()
    ...
    for col in genre_means.columns:
        genre_means[col] = (genre_means[col] - genre_means[col].mean()) / genre_means[
            col
        ].std()
    ...
    for col in user_feats.columns:
        user_feats[col] = (user_feats[col] - user_feats[col].mean()) / user_feats[
            col
        ].std()
    ...
    for col in item_feats.columns:
        item_feats[col] = (item_feats[col] - item_feats[col].mean()) / item_feats[
            col
        ].std()
